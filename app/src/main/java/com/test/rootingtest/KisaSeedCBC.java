...
public class KisaSeedCBC {
	...
	public static final byte[] key = {  0x01, 0x02, 0x03, 0x04,  0x05, 0x06, 0x07, 0x08,  0x09, 0x0A, 0x0B, 0x0C,  0x0D, 0x0E, 0x0F, 0x10 };
	public static final byte[] iv = {  0x01, 0x01, 0x01, 0x01,  0x01, 0x01, 0x01, 0x01,  0x01, 0x01, 0x01, 0x01,  0x01, 0x01, 0x01, 0x01 };
	...
	public String encrypt(String str){
		...
		try {
			...
			enc = KisaSeedCBC.SEED_CBC_Encrypt(key, iv, byteStr, 0, byteStr.length);
			Log.i("en", "message = "+str+", byte ="+enc);
		}catch (UnsupportedEncodingException e){
			return null;
		}

		byte[] encArray = Base64.encode(enc,Base64.DEFAULT);
		try {
	...
	public String decrypt(String str){
		...
		try {
			byte[] byteStr = str.getBytes("utf-8");
			enc = Base64.decode(byteStr,Base64.DEFAULT);
		} catch (UnsupportedEncodingException e) {
			return null;
		}

		enc = KisaSeedCBC.SEED_CBC_Decrypt(key, iv, enc, 0, enc.length);
		Log.i("de", "message = "+str+", byte ="+enc);
